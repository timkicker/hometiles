...
class IncomingCallScreenTest {
    ...
    private val service = Quelltext.withoutComments("dev/kicker/hometiles/phone/BigInCallService.kt")
    private val notices = Quelltext.withoutComments("dev/kicker/hometiles/phone/CallNotifications.kt")
    ...
    fun `the call channel does not carry the sms prefix`() {
        assertEquals(false, CallNotifications.CHANNEL.startsWith(dev.kicker.hometiles.sms.SmsNotifications.CHANNEL_PREFIX))
    ...
    fun `the reply rows may grow`() {
        ...
        val row = Quelltext.cut(
            Quelltext.file("dev/kicker/hometiles/phone/InCallActivity.kt").readText(),
