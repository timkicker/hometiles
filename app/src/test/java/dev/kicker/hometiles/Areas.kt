 ...
 * symbol really stands in. `dev.kicker.hometiles.phone.PhoneNumbers` lies in `:app` by its package name
...
object Areas {
    ...
    fun edges(without: Set<String> = emptySet()): Map<Pair<String, String>, List<String>> {
        ...
        files().filterNot { it.name in without }.forEach { file ->
            ...
            file.readLines().mapNotNull { line ->
                Regex("""^import (dev\.kicker\.hometiles\.[\w.]+)""").find(line.trim())?.groupValues?.get(1)
            }.forEach { full ->
    ...
    fun areaOf(file: File, root: File = Quelltext.appRoot): String {
        val parts = file.relativeTo(File(root, "dev/kicker/hometiles")).path.split(File.separator)
