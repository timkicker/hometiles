 ...
 *
 * two sentences did not, and both showed up on the emulator where HomeTiles holds the sms and
 * the dialer role: the hiding list said HomeTiles was not this phone's sms app while it was
 * storing the incoming messages, and the phone-state question said HomeTiles could not call
...
class DeviceClaimsTest {
    ...
    fun `the hint about the hiding list asks for the sms role`() {
        val place = source("dev/kicker/hometiles/sms/MessagesSettingsList.kt")
    ...
    fun `the text before the phone-state question looks at CALL_PHONE`() {
        val place = source("dev/kicker/hometiles/MainActivity.kt")
    ...
    fun `the hint about blocked numbers asks for the dialer role`() {
        val text = source("dev/kicker/hometiles/settings/SettingsActivity.kt")
        val place = text
    ...
    fun `the sentence about blinking matches what counts without access`() {
        val logic = source("dev/kicker/hometiles/notify/TileNotifications.kt")
