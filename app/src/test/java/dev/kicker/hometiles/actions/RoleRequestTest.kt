...
class RoleRequestTest {
    ...
    /**
     * "use as phone app" stood there even when HomeTiles already was one - a tap did visibly
     * nothing because the role dialog closed again at once.
     */
    @Test
    fun `the role rows name the state`() {
        val settings = Quelltext.file("dev/kicker/hometiles/settings/SettingsActivity.kt").readText()
    ...
    fun `both roles are asked for through a launcher`() {
        ...
        val wizard = Quelltext.file("dev/kicker/hometiles/wizard/WizardActivity.kt").readText()
