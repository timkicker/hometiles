 ...
 * for `SENDTO`. if **one** is missing, HomeTiles does not even appear in the choice of default
...
class SmsRoleTest {
    ...
    fun `the receiver for incoming messages is there`() {
        assertTrue(
            "SMS_DELIVER is missing - HomeTiles then never gets to see incoming messages",
            contains(
    ...
    fun `the service for replying during a call is there`() {
        assertTrue(
            "RESPOND_VIA_MESSAGE is missing - without it HomeTiles is not in the choice",
            contains(
