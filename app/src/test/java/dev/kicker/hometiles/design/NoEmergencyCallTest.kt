 ...
 *
 * HomeTiles's sos sends messages, no more - no call, least of all to 112. that was always so
 ...
 * [dev.kicker.hometiles.phone.PhoneNumbers] exist for the opposite - the dial pad should **recognise**
...
class NoEmergencyCallTest {
    ...
    fun `no part of the sos dials`() {
        ...
        assertEquals(
            "the sos dials here. HomeTiles's sos sends messages - a call from here can hit an " +
    ...
    fun `the preview does not reach the sending`() {
        ...
        val source = Quelltext.withoutComments("dev/kicker/hometiles/toggles/SosActivity.kt")
    ...
    fun `without a number nothing goes out`() {
        val send = Quelltext.file("dev/kicker/hometiles/toggles/Sos.kt").readText()
