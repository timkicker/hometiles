...
/**
 * every screen inherits from `HomeTilesActivity`.
 *
...
class ActivityBaseTest {
    ...
    @Test
    fun `every activity inherits from HomeTilesActivity`() {
        val foreign = activityLines()
            .filterNot { (file, _) -> file == "HomeTilesActivity.kt" }
            .filterNot { (_, line) -> line.endsWith(": HomeTilesActivity") }
            .map { "${it.first}: ${it.second}" }
        assertEquals(
            "these screens do not inherit from HomeTilesActivity and so lose orientation, " +
    ...
    @Test
    fun `HomeTilesActivity sets the orientation itself`() {
        val text = sources.first { it.name == "HomeTilesActivity.kt" }.readText()
