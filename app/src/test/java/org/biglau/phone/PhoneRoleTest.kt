...
/**
 * what BigLau has to bring along to be selectable as a phone app at all.
 *
 ...
 *
 * android demands an `ACTION_DIAL` entry for the phone role; without it BigLau does not even
...
class PhoneRoleTest {
    ...
    fun `there is an entry for ACTION_DIAL`() {
        ...
        assertTrue("ACTION_DIAL is missing - BigLau is then not up for choice", "android.intent.action.DIAL" in dialer)
    ...
    fun `the home screen registers itself as a home screen`() {
        ...
        ).forEach { assertTrue("$it is missing - without it BigLau is no home screen", it in main) }
    }
}
