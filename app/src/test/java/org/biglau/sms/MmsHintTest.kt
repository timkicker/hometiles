...
class MmsHintTest {
    ...
    private val receiver = Quelltext.file("org/biglau/sms/SmsComponents.kt").readText()
    private val notifications = Quelltext.file("org/biglau/sms/SmsNotifications.kt").readText()
     ...
     * app can fetch an mms, so once BigLau holds the role and does not fetch it, no other app
            ...
            deadEnds.forEach {
                assertTrue(
                    "$language: the hint sends to another messaging app. that one cannot " +
                        "fetch an mms while BigLau holds the role.",
                    it !in text,
                )
            }
        }
