...
/**
 * whatever BigLau did not build itself, it starts through [org.biglau.actions.Intents].
 *
 ...
 * "edit in the address book" without an address book app crashed BigLau.
 *
 * our own screens are exempt - a class from the same program always exists.
 */
class ForeignIntentTest {

    /** how one sees that the target is BigLau itself. */
     ...
     * hit BigLau; the call itself needs its own catch, since a `SecurityException` there
    ...
    fun `foreign intents run through Intents`() {
        ...
        assertTrue(
            "these calls start a foreign app without a catch - if it is missing, BigLau " +
    ...
    fun `the contact list reads again after the foreign editor`() {
        val source = Quelltext.withoutComments("org/biglau/contacts/ContactsActivity.kt")
     ...
     * list rather than back in BigLau - out of the big type. from a service or the
     ...
     */
    @Test
    fun `a foreign app starts in BigLau's task when there is one`() {
        val intents = Quelltext.withoutComments("org/biglau/actions/Intents.kt")
        ...
        assertTrue(
            "Intents hangs every foreign app in a task of its own; the back key then does " +
                "not lead back to BigLau:\n$start",
            start.contains("activityBehind(context) != null"),
        )
        val contacts = Quelltext.withoutComments("org/biglau/contacts/ContactRepository.kt")
        assertTrue(
