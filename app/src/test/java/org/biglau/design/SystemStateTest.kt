 ...
 *
 * `BigLauActivity.resumes` counts the returns; whoever reads such a state wraps it in
 * `remember(resumes.intValue) { … }`.
 */
class SystemStateTest {

    private val base = Quelltext.file("org/biglau/ui/BigLauActivity.kt").readText()

    @Test
    fun `the shared base counts the returns`() {
        assertTrue("BigLauActivity no longer knows `resumes`", "resumes" in base)
    ...
    fun `notification access is read again on coming back`() {
        val place = Quelltext.file("org/biglau/settings/SettingsActivity.kt").readText()
    ...
    fun `the wizard reads its state again on coming back`() {
        val place = Quelltext.file("org/biglau/wizard/WizardActivity.kt").readText()
     ...
     * permanently denied that way is gone, BigLau sends the user to `Intents.appSettings`,
