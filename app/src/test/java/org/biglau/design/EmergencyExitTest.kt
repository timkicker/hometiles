...
/**
 * the emergency exit leads out of BigLau, not back into it.
 *
 ...
 * is BigLau itself. the emergency exit would have led back into the app that just crashed
 * twice.
 *
...
class EmergencyExitTest {
    ...
    private val emergency = Quelltext.withoutComments("org/biglau/safety/EmergencyScreen.kt")
    ...
    fun `the emergency exit asks explicitly for a foreign app`() {
        ...
        assertTrue(
            "openDialer sends a bare ACTION_DIAL. that goes to the default phone app - and " +
                "if BigLau is it, the emergency exit leads back into the crashed app.",
