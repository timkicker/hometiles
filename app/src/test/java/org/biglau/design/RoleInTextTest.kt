 ...
 * once BigLau holds the role and does not fetch it, no other app can either. a consolation
...
class RoleInTextTest {
    ...
    private val page = Quelltext.withoutComments("org/biglau/sms/MessagesSettingsList.kt")
            ...
            listOf("sms_no_mms_default", "mms_arrived_body").forEach { name ->
                assertEquals(
                    "$language/$name ends word for word like sms_no_mms. that sentence is " +
                        "only true while **another** app holds the role; here BigLau holds " +
