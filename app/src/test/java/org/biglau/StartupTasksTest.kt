 ...
 *
 * removing the call gives **no** error: BigLau then writes phone numbers in bare groups of
...
class StartupTasksTest {
    ...
    fun `the start hooks up the system parts of the phone numbers`() {
        assertTrue(
            "BigLauApp no longer calls SystemNumbers.install. without the call BigLau writes " +
     ...
     * cost another permission, and BigLau **is** the home screen: it runs after every restart
     * anyway.
     *
    ...
    fun `the start sets the reminder alarm again`() {
        assertTrue(
            "BigLauApp no longer sets the reminder alarm - after a restart nothing reminds " +
                "of an unread message any more.",
            "MessageReminderReceiver.schedule" in start,
        )
        assertTrue(
            "without the check on SmsReminder.active BigLau sets an alarm at every start " +
                "that nobody ordered",
    ...
    fun `the start does not touch telephony itself`() {
        val framework = listOf("TelephonyManager", "PhoneNumberUtils")
            .filter { it in start }
        assertTrue(
            "BigLauApp touches android telephony directly: $framework. that belongs in " +
