 ...
 *   "unbekannt" says that.
 *
 * plus an inequality in the same section: the off case said "Für BigLau aus", the on case
 * only "An" - which reads as the phone's own saving mode. both name BigLau now.
 */
class DiagnosticWordsTest {

    private val source = Quelltext.withoutComments("org/biglau/settings/Diagnostics.kt")
    ...
    fun `both sides of the battery line are equally wide`() {
        listOf("values-de", "values").forEach { language ->
            ...
            val namesApp = { text: String -> "BigLau" in text }
            assertTrue(
                "only one of the two answers names BigLau (language \"$language\"): " +
