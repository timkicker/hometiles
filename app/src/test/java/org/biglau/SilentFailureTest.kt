...
class SilentFailureTest {
    ...
    fun `no attempt ends silently in ToggleActions`() {
        val lines = Quelltext.file("org/biglau/toggles/ToggleActions.kt").readLines()
    ...
    fun `no result from InCallRepository is thrown away`() {
        ...
        val withResult = Quelltext.file("org/biglau/phone/InCallRepository.kt")
            .readLines()
            .mapNotNull { line ->
    ...
    fun `the call screen reports a failure`() {
        val source = Quelltext.file("org/biglau/phone/InCallActivity.kt").readText()
        assertTrue(
