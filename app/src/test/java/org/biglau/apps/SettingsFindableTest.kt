 ...
 *
 * on the user's device it was on 3.9.2026 the only way into the BigLau settings, and it stood
...
class SettingsFindableTest {
    ...
    fun `the row no longer hangs on an empty search field`() {
        val source = Quelltext.file("org/biglau/apps/AppDrawerActivity.kt").readText()
    ...
    fun `whoever searches for the settings finds them`() {
        listOf("values" to "setting", "values-de" to "einstell").forEach { (language, word) ->
            ...
            assertNotNull("$language: $label is not hit by biglau", TextSearch.rank(label, "biglau"))
        }
    }
