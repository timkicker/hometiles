...
import dev.kicker.hometiles.apps.AppLock
import dev.kicker.hometiles.security.Pin
import dev.kicker.hometiles.sms.SmsRepository
import dev.kicker.hometiles.ui.bigSp
import dev.kicker.hometiles.ui.PinGate
import dev.kicker.hometiles.ui.SystemBarsEffect
import java.util.Locale
import dev.kicker.hometiles.ui.AppLocale
import dev.kicker.hometiles.ui.absorbTouches
...
import dev.kicker.hometiles.apps.AppDrawerActivity
...
import dev.kicker.hometiles.data.PressMode
...
import dev.kicker.hometiles.safety.CrashGuard
import dev.kicker.hometiles.settings.Reset
import dev.kicker.hometiles.safety.CrashRecorder
import dev.kicker.hometiles.safety.EmergencyScreen
import dev.kicker.hometiles.safety.StartMode
import dev.kicker.hometiles.contacts.ContactsActivity
import dev.kicker.hometiles.data.ContactMode
import dev.kicker.hometiles.info.BatteryRepository
import dev.kicker.hometiles.info.SignalRepository
import dev.kicker.hometiles.notify.NotificationRepository
import dev.kicker.hometiles.phone.CallLogRepository
import dev.kicker.hometiles.phone.DialerActivity
import dev.kicker.hometiles.toggles.SosActivity
import dev.kicker.hometiles.ui.Notice
import dev.kicker.hometiles.wizard.WizardActivity
import dev.kicker.hometiles.wizard.WizardSteps
import dev.kicker.hometiles.toggles.ToggleActions
import dev.kicker.hometiles.toggles.ToggleKind
import dev.kicker.hometiles.notify.SystemPackagesReader
import dev.kicker.hometiles.settings.SettingsActivity
import dev.kicker.hometiles.sms.SmsActivity
import dev.kicker.hometiles.widgets.WidgetHostController
import dev.kicker.hometiles.shortcuts.ShortcutRepository
import dev.kicker.hometiles.tiles.ScreenOrder
import dev.kicker.hometiles.tiles.SwipeGesture
import androidx.compose.ui.platform.LocalDensity
import androidx.compose.foundation.gestures.detectHorizontalDragGestures
import dev.kicker.hometiles.web.LinkTarget
import dev.kicker.hometiles.tiles.TileEditorActivity
import dev.kicker.hometiles.tiles.TileLabel
import dev.kicker.hometiles.a11y.TileMenu
import dev.kicker.hometiles.a11y.LongPress
import dev.kicker.hometiles.a11y.MenuItem
import dev.kicker.hometiles.a11y.LongPressAction
import dev.kicker.hometiles.a11y.Speaker
import dev.kicker.hometiles.ui.HomeHeader
import dev.kicker.hometiles.ui.labelRes
import dev.kicker.hometiles.ui.HomeScreenView
import dev.kicker.hometiles.ui.theme.HomeTilesTheme
import dev.kicker.hometiles.ui.theme.LocalBigPalette

class MainActivity : HomeTilesActivity() {
    ...
    /**
     * an app waiting for the pin. `PLAN.md` 4.5, see [dev.kicker.hometiles.apps.AppLock]. on the
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val perform: (dev.kicker.hometiles.data.Screen, Int, Int, List<LongPressAction>) -> Unit =
                { shown, x, y, actions ->
            ...
            HomeTilesTheme(
                ...
                Column(
                    ...
                    // ask again on every return: tapping the banner picks HomeTiles in the
    ...
    private fun labelAt(
        config: dev.kicker.hometiles.data.LauncherConfig,
        screenId: String,
        x: Int,
        y: Int,
        apps: AppRepository,
    ...
    Box(
        ...
        Column(
            ...
            Text(
                ...
                fontSize = dev.kicker.hometiles.ui.dpSp(44f),
                fontWeight = FontWeight.Bold,
            ...
            Text(
                ...
                fontSize = dev.kicker.hometiles.ui.dpSp(16f),
                textAlign = TextAlign.Center,
            )
        }
    }
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = name,
                color = palette.onBackground,
                fontSize = dev.kicker.hometiles.ui.dpSp(30f),
                fontWeight = FontWeight.Bold,
                maxLines = 2,
            ...
            Text(
                ...
                color = palette.onBackground.copy(alpha = 0.75f),
                fontSize = dev.kicker.hometiles.ui.dpSp(15f),
                modifier = Modifier.padding(horizontal = 4.dp),
            )
        }
    }
...
private fun FolderOverlay(
    ...
    Column(
        ...
        Text(
            text = name,
            color = palette.onBackground,
            fontSize = dev.kicker.hometiles.ui.dpSp(26f),
            fontWeight = FontWeight.Bold,
            maxLines = 1,
 ...
 * group, which sounds like far more than is needed here: HomeTiles wants the number of bars
...
private fun SignalPermissionExplainer(
    ...
    Column(
        ...
        if (blocked) {
            Text(
                text = stringResource(dev.kicker.hometiles.core.ui.R.string.permission_blocked),
                color = palette.dangerText,
                fontSize = dev.kicker.hometiles.ui.dpSp(15f),
                modifier = Modifier.padding(horizontal = 4.dp, vertical = 4.dp),
            )
            BigRow(
                label = stringResource(dev.kicker.hometiles.core.ui.R.string.permission_open_settings),
                surface = palette.surfaceAccent,
                modifier = Modifier
                    .focusRequester(anchors[0])
                    .onFocusChanged { if (it.isFocused) at = 0 },
