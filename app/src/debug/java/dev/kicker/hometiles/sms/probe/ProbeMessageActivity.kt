...
import dev.kicker.hometiles.sms.SmsDeliverReceiver
import dev.kicker.hometiles.sms.WapPushDeliverReceiver

/**
 * lays a message before HomeTiles that never came through a radio net.
 *
 *     adb shell am start -n dev.kicker.hometiles.debug/dev.kicker.hometiles.sms.probe.ProbeMessageActivity
 ...
 * step in HomeTiles on: same class, same intent, same reading through
 * `Telephony.Sms.Intents.getMessagesFromIntent`.
 *
 * what this does **not** check: that android hands the broadcast to HomeTiles at all. only a
...
class ProbeMessageActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent?.getStringExtra("was") == "mms") {
            Log.i("HomeTiles", "probe: mms notice")
        ...
        Log.i("HomeTiles", "probe message: ${pdu.size} bytes from ${ProbeMessage.SENDER}")
    ...
    private fun clear() {
        ...
        Log.i("HomeTiles", "probe messages cleared: $removed")
    }
}
