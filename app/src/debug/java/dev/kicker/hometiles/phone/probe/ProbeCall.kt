 ...
 * a call nobody makes.
 *
 * the call screen is the one part of HomeTiles one cannot try out without ringing somebody - and
 * the one where a fault means one cannot pick up. so HomeTiles places the call itself: an own
...
object ProbeCall {
    ...
    /**
     * the id of the account telecom runs the probe under. it went along with the rename on
     * 06.09.2026: the account is registered again at every start, and the one left behind
     * belongs to the old package, which is gone.
     */
    const val ACCOUNT = "hometiles-probe"
     ...
     *
     * the value is a bundle key inside the app, so it went along with the rename on
     * 06.09.2026. what stays is the adb extra `klingeln`, which is typed by hand, PLAN.md 11.4.
     */
    const val EXTRA_RINGING = "dev.kicker.hometiles.probe.KLINGELN"
