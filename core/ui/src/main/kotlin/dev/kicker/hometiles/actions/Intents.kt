...
object Intents {
    ...
    /**
     * a keypad that is explicitly *not* HomeTiles, without a number. `ACTION_DIAL` never dials
     * by itself, it only opens.
     *
     * this is the safety net for two failed starts in a row, and a bare `ACTION_DIAL` goes
     * to the default phone app: once that is HomeTiles, the emergency exit led back into the
     * app that had just crashed twice. without another one the bare intent stays, since a
     * keypad that might be HomeTiles still beats none.
     */
    ...
    /** only on an explicit move: the role hands HomeTiles the call screen, and a fault there ends calling. */
    ...
    private inline fun start(context: Context, build: () -> Intent) {
        ...
        // belongs *in* HomeTiles's task, or back does not lead back and one ends up in the
