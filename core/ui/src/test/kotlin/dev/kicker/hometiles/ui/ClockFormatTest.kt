...
class ClockScaleTest {
    ...
    fun `the default changes nothing`() {
        assertEquals(1.0f, dev.kicker.hometiles.data.Appearance().clockScale, 0.001f)
    ...
    fun `the default is on offer too`() {
        assertEquals(true, ClockFormat.SCALES.contains(dev.kicker.hometiles.data.Appearance().clockScale))
    }
